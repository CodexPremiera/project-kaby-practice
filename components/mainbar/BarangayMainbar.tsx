...
const NAVIGATION_ITEMS = [
	{ name: "Home", path: `/barangay`, icon: RiCommunityLine },
	{ name: "Citizen Desk", path: `/barangay/citizen_desk`, icon: RiTableLine },
	{ name: "Service Desk", path: `/barangay/service_desk`, icon: RiInboxLine },
	{ name: "Settings", path: `/barangay/account`, icon: RiSettings4Line },
];
	...
	const renderLink = (link: {
		...
		return (
			<Link key={link.path} href={link.path}>
				<div
					className={`flex flex-col items-center justify-center px-3 py-3 mt-2 cursor-pointer group transition-all duration-300 ${
	...
	return (
		<div className="z-10 fixed left-0 bottom-0 w-full sm:top-16 sm:bottom-0 sm:left-0 sm:w-[75px] sm:h-full flex sm:flex-col flex-row justify-center items-center px-2  sm:px-0  bg-white text-black">
			<div className="flex flex-1 w-full sm:flex-col flex-row sm:justify-start justify-around items-center sm:space-y-4 sm:pt-4 sm:pb-8 px-4 sm:px-0">
				{NAVIGATION_ITEMS.map((link) => renderLink(link))}
			</div>
		</div>
	);
};
