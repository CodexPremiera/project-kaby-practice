				...
				{NAVIGATION_ITEMS.map((link) => {
					...
					return (
						<Link
							key={link.path}
							href={link.path}
							className={`flex flex-col items-center justify-center px-3 py-3 mt-2 cursor-pointer group rounded-[10px] transition-all duration-300 
              ${isActive ? "bg-secondary/10" : "hover:bg-secondary-/10"}`}
						>
